[![PyPI version](https://img.shields.io/pypi/v/fastshare?v=1)](https://pypi.org/project/fastshare/)
[![Python versions](https://img.shields.io/pypi/pyversions/fastshare?v=1)](https://pypi.org/project/fastshare/)
[![License](https://img.shields.io/pypi/l/fastshare?v=1)](https://github.com/AwaisAdilKhokhar/fastshare/blob/master/LICENSE)
[![CI](https://img.shields.io/github/actions/workflow/status/AwaisAdilKhokhar/fastshare/ci.yml?branch=master&v=1)](https://github.com/AwaisAdilKhokhar/fastshare/actions)
...
| `bytes` | 1 GB | 531 ms | 1,645 ms | 0.32x |
| `bytearray` | 1 GB | 534 ms | 1,577 ms | 0.34x |
...
supports pickle protocol 5 out-of-band buffers with efficient zero-copy
reconstruction (NumPy arrays). At 1 GB, NumPy arrays are 1.8x faster because
zero-copy deserialization avoids the full-array copy that `pickle.loads()` must
perform. Raw `bytes` and `bytearray` objects, despite supporting out-of-band
buffers, don't benefit in single-process round-trips -- the shared memory
`memcpy` overhead outweighs the savings. Arrow Tables show similar overhead
(Arrow IPC serialization cost). However, in multi-process scenarios the advantage
compounds for all types -- shared memory avoids the additional pipe-copy overhead
that `multiprocessing.Queue` incurs, and broadcast to N workers amortizes the
single write across all readers.
